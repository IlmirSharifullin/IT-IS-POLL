    ...
    if (!token || !user) {
        return (
            <div style={{padding: 40, textAlign: 'center', fontSize: 20}}>
                Только авторизованные пользователи могут создавать опросы.
            </div>
        );
    }
                    ...
                    <div className="survey-create__questions" aria-live="polite" aria-relevant="additions removals">
                        {questions.map((q, idx) => (
                            <div className="survey-create__question" key={idx}>
                                ...
                                <label className="survey-create__label" htmlFor={`question-type-${idx}`}>Тип
                                    вопроса</label>
                                <select
            ...
            <div className="survey-create__preview">
                ...
                {questions.map((q, idx) => (
                    <div className="survey-create__preview-question" key={idx}>
                        ...
                        {q.type === 'text' &&
                            <div className="survey-create__preview-option" style={{opacity: 0.7}}>Текстовый ответ</div>}
                    </div>
                ))}
            </div>
        </div>
